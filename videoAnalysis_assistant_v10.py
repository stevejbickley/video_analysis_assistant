...
def split_audio(mp3_io, max_size_bytes=26214400):
    """
    Splits the audio BytesIO into two parts if its size exceeds max_size_bytes.
    ...
    first_half = audio[:duration_ms // 2]
    second_half = audio[duration_ms // 2:]
    # Export each half to a BytesIO object
    first_io = io.BytesIO()
    second_io = io.BytesIO()
    first_half.export(first_io, format="mp3")
    second_half.export(second_io, format="mp3")
    first_io.seek(0)
    first_io.name = "audio_part1.mp3"
    second_io.seek(0)
    second_io.name = "audio_part2.mp3"
    return [first_io, second_io]


def transcribe_audio(video_path, segment_timing=True, max_size_bytes=26214400):
    """
    Extracts audio from a video file and transcribes it using the Whisper API.
    If the audio file exceeds the size limit, splits it into parts, transcribes each part,
    and combines the resulting segments.
    """
    ...
    if len(mp3_io.getvalue()) > max_size_bytes:
        # Split the audio into parts.
        parts = split_audio(mp3_io, max_size_bytes)
        transcriptions = []
        for part in parts:
            part.seek(0)
        ...
        # Combine segments from each transcription.
        combined_segments = []
        for transcription in transcriptions:
            combined_segments.extend(transcription.segments)
        # Create a simple object to mimic the transcription result.
        class CombinedTranscription:
            def __init__(self, segments):
                self.segments = segments
        return CombinedTranscription(combined_segments)
    else:
...
def extract_frames_by_seconds(video_path, secs_per_image=1):
    frames = []
    cap = cv2.VideoCapture(video_path)
    fps = cap.get(cv2.CAP_PROP_FPS)
    frame_count = int(cap.get(cv2.CAP_PROP_FRAME_COUNT))
    ...
    duration = frame_count / fps
    current_time=0.0
    while current_time <= duration:  # replaces the for loop below
    #for sec in range(int(duration) + 1):
        ...
        if ret:
            ...
            print(f"Warning: Could not read frame at {current_time:.2f} seconds")
        current_time+=secs_per_image
    cap.release()
    return frames
...
#allfiles = glob.glob(INPUT_PATH + '*Recording.mp4')
...
# Prompt for OpenAI API
prompt = """These attached images are frames from an online video/recording, accompanied by an audio transcript in JSON format.
Generate a comprehensive historical action/activity/event log detailing everything that happens in each scene in a structured table.
For each event, also include:
- Scene/background context.
- Any hidden messages or 'reading between the lines' moments.
- Any use of humor, if present.
At the end of the table, also provide an overall summary or qualitative description of the video.
Please provide the response in JSON format."""

# Process each video file one-by-one
for video_path in allfiles:
    if os.path.exists(OUTPUT_PATH + video_path.split('/')[-1][:-4] + '_audioLog.csv'):  # CHANGE for euro / champions / europa / world cup
        continue
    # Extract frames from the video
    #frames = extract_frame_by_frame(video_path) # old/superseded code
    frames = extract_frames_by_seconds(video_path, 0.5) # take a screenshot every 0.5 seconds
    if check_if_all_arrays_sum_to_zero(frames):
        frames = extract_frames_entire_video(video_path, 0.5)
        if check_if_all_arrays_sum_to_zero(frames):
