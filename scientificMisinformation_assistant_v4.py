...
# Define JSON schema to include fields corresponding to each typology with binary and explanation fields
json_schema = {
    "type": "object",
    "properties": {
        "False_Connection": {
...
def analyze_misinformation_with_files_completionsAPI(original_pdf_path, media_pdf_path, api_key, model="gpt-4o-mini"):
    """
...
def analyze_misinformation_with_files_assistantsAPI(client, original_pdf_path, media_pdf_path, assistant_id, model="gpt-4o-mini", formatted_response="no"):
    """
    Analyzes a news article for misinformation based on a scientific article using uploaded PDF files.
    Args:
    ...
    """
    # Upload the original and media PDFs
    original_file = client.files.create(file=open(original_pdf_path, "rb"), purpose="assistants")
    media_file = client.files.create(file=open(original_pdf_path, "rb"), purpose="assistants")
    # Construct the prompt referring to the uploaded files
    prompt = (
        f"Given the original scientific article uploaded as {original_file.id} "
        f"and the news article uploaded as {media_file.id}, identify and categorize any misinformation present "
        f"in the news article according to the following typology:\n\n"
        f"False Connection, Misleading Content, False Context, Fabrication, "
        f"Satire or Parody, Manipulation, Propaganda, Biased, Inaccurate Information, "
        f"Unsubstantiated Claims, Exaggeration.\n\n"
        ...
        messages=[
            {
                "role": "user",
                "content": prompt,
                "attachments": [{"file_id": original_file.id, "tools": [{"type": "file_search"}]},
                                {"file_id": media_file.id, "tools": [{"type": "file_search"}]}],
            }
        ]
    )
    ...
    # Retrieve the messages and parse the output
    messages = list(client.beta.threads.messages.list(thread_id=thread.id, run_id=run.id))
    message_content = messages[0].content[0].text
    message_string = message_content.value
    annotations = message_content.annotations  # Use the correct content indexing
    #citations = []
...
def extract_json_from_string(response_string):
    """
    Extracts a JSON dictionary from a string that may contain text along with a JSON object.
    Args:
    - response_string (str): The string containing the JSON object.
    Returns:
    - dict: The extracted JSON as a Python dictionary, or an empty dictionary if extraction fails.
    """
    try:
...
def analyze_misinformation_output(result):
    """
    Extracts the JSON dictionary from the 'string' field of the result or directly from the text.
    Args:
    - result (dict): The result dictionary containing the analysis response.
    Returns:
    - dict: The extracted JSON as a Python dictionary, or an empty dictionary if extraction fails.
    """
    if 'string' in result:
        return extract_json_from_string(result['string'])
    if 'analysis' in result and isinstance(result['analysis'], str):
        return extract_json_from_string(result['analysis'])
    print("No valid field found for extracting JSON.")
    return {}
...
# Use the analyze_misinformation_with_texts function to get the result
...
# Create new assistant without response_format (i.e. can't currently use json_schema with file_search tools
assistant = client.beta.assistants.create(
  name="Scientific Misinformation Analyst Assistant",
  instructions="You are an expert in analyzing and debunking scientific misinformation. Use your extensive knowledge base to critically assess the accuracy and reliability of scientific claims, identify potential misinformation, and provide evidence-based responses.",
  model="gpt-4o-2024-08-06",
  tools=[{"type": "file_search"}],
)

# Use the analyze_misinformation_with_files function to get the result
result2 = analyze_misinformation_with_files_assistantsAPI(client, original_pdf_path, media_pdf_path, assistant_id=assistant.id, model="gpt-4o-2024-08-06", formatted_response="yes")
extracted_json = analyze_misinformation_output(result2)
result2_df = pd.DataFrame.from_dict(extracted_json)
