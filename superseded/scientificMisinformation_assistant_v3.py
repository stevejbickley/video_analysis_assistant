...
def analyze_misinformation_with_files_assistantsAPI(client, original_pdf_path, media_pdf_path, assistant_id, model="gpt-4o-mini"):
    """
    Analyzes a news article for misinformation based on a scientific article using uploaded PDF files.
    Args:
    ...
    message_content = str(messages[0].content[0].text)  # Convert to string explicitly
    annotations = messages[0].content[0].annotations  # Use the correct content indexing
    citations = []
...
result2 = analyze_misinformation_with_files_assistantsAPI(client, original_pdf_path, media_pdf_path, assistant_id=assistant.id, model="gpt-4o-2024-08-06")
